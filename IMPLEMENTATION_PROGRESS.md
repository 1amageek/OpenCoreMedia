...
and its own small readiness state. The native Smoke verifies object identity and
shared mutation through borrowed pixel-buffer byte access: writes through the
source, original sample, and timing-only copy are observed from the same storage.
...
- Native:
  ...
  — passed 32 behavior tests on 2026-07-24.
...
- Embedded WASM:
  ...
  — passed without warnings after `swift package clean` on 2026-07-24.
